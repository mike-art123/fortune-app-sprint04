...
abstract final class EnvironmentLoader {
  static AppConfig load(AppFlavor flavor) {
    ...
    const connectMs = int.fromEnvironment('API_CONNECT_TIMEOUT_MS', defaultValue: 15000);
    const receiveMs = int.fromEnvironment('API_RECEIVE_TIMEOUT_MS', defaultValue: 20000);
    const analytics = bool.fromEnvironment('ENABLE_ANALYTICS', defaultValue: false);
    const crash = bool.fromEnvironment('ENABLE_CRASH_REPORTING', defaultValue: false);
    const debugMenu = bool.fromEnvironment('ENABLE_DEBUG_MENU', defaultValue: false);
