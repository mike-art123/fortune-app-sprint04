...
describe('wallet (e2e) — Sprint 04 identity + debit/refund economy', () => {
  ...
  describe('readings no longer charge coins', () => {
    it('a reading creates no debit and leaves the balance unchanged', async () => {
      ...
      expect(after.body.data.balance).toBe(startBalance);
    });

    it('the same Idempotency-Key replays the same reading and never debits', async () => {
    ...
    it('concurrent identical requests replay safely and never debit', async () => {
    ...
    it('many free readings all succeed and never touch the balance', async () => {
      const session = await loginAs(app, { id: freshTelegramId(), first_name: 'رایگان‌ها' });
