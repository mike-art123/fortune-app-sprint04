import { TarotReadingProvider, parseTarotReading } from './tarot-reading.provider';
...
describe('parseTarotReading', () => {
  it('accepts a reply with every section', () => {
    const parsed = parseTarotReading(VALID_REPLY);
  ...
  it('refuses a reply missing a section', () => {
    ...
    delete reply.hope;
    expect(() => parseTarotReading(JSON.stringify(reply))).toThrow('hope');
  });

  it('refuses a completion that is not JSON', () => {
    expect(() => parseTarotReading('نه یک شیء JSON')).toThrow();
  });
});

describe('TarotReadingProvider', () => {
  ...
  it('passes every other fortune straight through', async () => {
    ...
    const provider = new TarotReadingProvider(
      inner,
      makeFlags(true),
      makeConfig(),
      monetization,
  ...
  it('passes tarot through while the flag is off', async () => {
    const inner = makeInner();
    const provider = new TarotReadingProvider(
      inner,
      makeFlags(false),
    ...
    await provider.generate(tarot, { intention: 'سلامتی' });
  ...
  it('serves the drawn card: real card in the prompt, same card in the reading', async () => {
    ...
    const out = await provider.generate(tarot, { intention: 'سلامتی' });

    expect(inner.generate).not.toHaveBeenCalled();
    expect(out.title).toMatch(/^تاروت — /);
    ...
    const cardName = out.title.replace('تاروت — ', '').replace(' (وارونه)', '');
  ...
  it('draws the same card for the same intention on the same day', async () => {
    ...
    const a = await provider.generate(tarot, { intention: 'سلامتی' });
    const b = await provider.generate(tarot, { intention: 'سلامتی' });
  ...
  it('fails honestly on an upstream error instead of answering canned', async () => {
    ...
    await expect(provider.generate(tarot, { intention: 'نیت' })).rejects.toThrow('503');
    expect(inner.generate).not.toHaveBeenCalled();
    expect(logger.lines.join(' ')).toContain('reading.tarot.failed');
  });
  ...
  it('never writes the offering into the logs', async () => {
    ...
    await expect(provider.generate(tarot, { intention: secret })).rejects.toThrow();
