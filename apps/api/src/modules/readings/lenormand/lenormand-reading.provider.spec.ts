import { LenormandReadingProvider, parseLenormandReading } from './lenormand-reading.provider';
...
const lenormand = findFortune('lenormand')!;
...
describe('parseLenormandReading', () => {
  it('accepts a reply with every section', () => {
    const parsed = parseLenormandReading(VALID_REPLY);
  ...
  it('refuses a reply missing a section', () => {
    ...
    delete reply.hope;
    expect(() => parseLenormandReading(JSON.stringify(reply))).toThrow('hope');
  });

  it('refuses a completion that is not JSON', () => {
    expect(() => parseLenormandReading('نه یک شیء JSON')).toThrow();
  });
});

describe('LenormandReadingProvider', () => {
  ...
  it('passes every other fortune straight through', async () => {
    ...
    const provider = new LenormandReadingProvider(
      inner,
      makeFlags(true),
      makeConfig(),
      monetization,
  ...
  it('passes lenormand through while the flag is off', async () => {
    const inner = makeInner();
    const provider = new LenormandReadingProvider(
      inner,
      makeFlags(false),
    ...
    await provider.generate(lenormand, { intention: 'سلامتی' });
  ...
  it('serves the drawn card: real card in the prompt, same card in the reading', async () => {
    ...
    const out = await provider.generate(lenormand, { intention: 'سلامتی' });

    expect(inner.generate).not.toHaveBeenCalled();
    expect(out.title).toMatch(/^لنورماند — /);
    ...
    const cardName = out.title.replace('لنورماند — ', '');
  ...
  it('draws the same card for the same intention on the same day', async () => {
    ...
    const a = await provider.generate(lenormand, { intention: 'سلامتی' });
    const b = await provider.generate(lenormand, { intention: 'سلامتی' });
  ...
  it('fails honestly on an upstream error instead of answering canned', async () => {
    ...
    await expect(provider.generate(lenormand, { intention: 'نیت' })).rejects.toThrow('503');
    expect(inner.generate).not.toHaveBeenCalled();
    expect(logger.lines.join(' ')).toContain('reading.lenormand.failed');
  });
  ...
  it('never writes the offering into the logs', async () => {
    ...
    await expect(provider.generate(lenormand, { intention: secret })).rejects.toThrow();
