...
export const LENORMAND_RAW_ENGINE_FLAG = 'lenormand.raw-engine';
...
class LenormandRequestError extends Error {
  constructor(
    message: string,
    readonly retryable: boolean,
  ) {
    super(message);
    this.name = 'LenormandRequestError';
  }
}

/** The model's half of the schema; the card and its meaning are ours to draw
 *  and add. */
interface LenormandModelReading {
...
/**
 * The Lenormand raw engine, worn as a decorator over the ordinary provider so
 * nothing outside this folder learns a new name. Every fortune that is not
 * lenormand — and lenormand itself while the flag is off or no model is
 ...
 */
@Injectable()
export class LenormandReadingProvider implements ReadingProvider {
  constructor(
  ...
  async generate(
    ...
    if (fortune.id !== 'lenormand') {
      return this.inner.generate(fortune, input, profile);
    }
    if (!(await this.flags.isEnabled(LENORMAND_RAW_ENGINE_FLAG))) {
      return this.inner.generate(fortune, input, profile);
    }
    if (!this.config.isConfigured) {
      this.logger.warn('reading.lenormand.unconfigured', {
    ...
    const index = drawLenormandCard({
      ...
      deckSize: LENORMAND_DECK.length,
    });
    const card = LENORMAND_DECK[index];
    if (!card) {
      ...
      throw new LenormandRequestError(`draw fell outside the deck at ${index}`, false);
    }
    const messages = buildLenormandPrompt(card, input, profile);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const parsed = parseLenormandReading(content);
        this.logger.info('reading.lenormand.succeeded', {
          cardId: card.id,
          attempt,
        ...
        this.logger.warn('reading.lenormand.retrying', {
          cardId: card.id,
          attempt,
    ...
    this.logger.error('reading.lenormand.failed', {
      cardId: card.id,
    ...
    throw lastError instanceof Error
      ? lastError
      : new LenormandRequestError('lenormand generation failed for an unknown reason', false);
  }
   ...
   */
  private async requestOnce(messages: ReturnType<typeof buildLenormandPrompt>): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        if (MISCONFIGURED_STATUS.has(response.status)) {
          this.logger.error('reading.lenormand.misconfigured', {
            status: response.status,
            model: this.config.model,
          });
        }
        throw new LenormandRequestError(
      ...
      if (!content) {
        throw new LenormandRequestError('upstream returned an empty completion', true);
      }
      return content;
    } catch (error) {
      if (error instanceof LenormandRequestError) throw error;
      if (controller.signal.aborted) {
        throw new LenormandRequestError(`timed out after ${this.config.timeoutMs}ms`, true);
      }
      throw new LenormandRequestError(
 ...
 */
export function parseLenormandReading(raw: string): LenormandModelReading {
  ...
  const section = (key: keyof LenormandModelReading): string => {
    const value = object[key];
    const text = typeof value === 'string' ? value.trim() : '';
    if (!text) {
      throw new LenormandRequestError(`completion is missing ${key}`, true);
    }
